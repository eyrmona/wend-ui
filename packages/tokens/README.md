...
**Exception: `font-family-base` doesn't push as-is.** Its value is a CSS fallback stack (`-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif`) — a browser-only concept where the OS picks whichever font is installed. Figma text nodes need one concrete, installed font, so there's no literal translation. By convention, the Figma-side `font-family-base` variable holds `"Inter"` instead — a deliberate single-font stand-in chosen for its visual similarity to native system UI fonts, not a build output. Treat a `font-family-base` diff between code (`"-apple-system, ..."`) and Figma (`"Inter"`) as expected, not drift.
